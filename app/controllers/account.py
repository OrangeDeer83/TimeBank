...
def GM_verify(token):
    data = validate_token(current_app.config['SERECT_KEY'], token)
    if data:
        try:
            query_data = adminAccount.query.filter(adminAccount.adminID == func.binary(data['GMID'])).first()
        except:
            return redirect(url_for('GM.verify', result='資料庫錯誤，請稍後再試'))
        if query_data:
            if query_data.adminType == userType['GM_apply']:
                try:
                    ...
                    return redirect(url_for('GM.verify', result='資料庫錯誤，請稍後再試'))
                return redirect(url_for('GM.verify', result='帳號驗證成功，等待管理員審核'))
                ...
                try:
                    ...
                    return redirect(url_for('GM.verify', result='資料庫錯誤，請稍後再試'))
                return redirect(url_for('GM.verify', result='帳號驗證成功，歡迎加入我們'))
            elif query_data.adminType == userType['GM'] or query_data.adminType == userType['GM_waiting']:
                return redirect(url_for('GM.verify', result='帳號已驗證成功，不需再次驗證'))
        else:
            return redirect(url_for('GM.verify', result='帳號不在資料庫中，請重新申請'))
    else:        
        return redirect(url_for('GM.verify', result='帳號驗證失敗'))
